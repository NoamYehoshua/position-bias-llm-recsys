...
The repository is intentionally kept lightweight. Source code, documentation, and dependency files are tracked, while large/generated data artifacts are created locally when the pipeline runs.
...
├── data_prep.py
├── judge_eval.py
├── Small LLM Judge - Position Bias.pdf
├── .gitignore
│
└── data/                         # generated locally after running the scripts
    ├── ml-1m/                    # extracted MovieLens 1M dataset
    ├── ml-1m.zip                 # downloaded MovieLens archive
    ├── evaluation_dataset.csv    # generated slate-pair dataset
...
| File | Description |
|---|---|
| `README.md` | Project documentation, setup instructions, usage guide, and research summary. |
...
| `Small LLM Judge - Position Bias.pdf` | Final research report summarizing the experiment, methodology, findings, limitations, and future work. |
| `.gitignore` | Prevents local secrets, cache files, and large/generated artifacts from being committed. |
| `data/evaluation_dataset.csv` | Generated locally by `data_prep.py`. Used as input for the judge evaluation script. |
| `data/evaluation_results_groq.csv` | Generated locally by `judge_eval.py`. Stores the full evaluation output. |

> Note: The `data/` directory is generated by the code and may not appear immediately after cloning the repository. Run `python data_prep.py` first to create the dataset.
